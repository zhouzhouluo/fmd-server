...
//强制嵌入框架中
/* 
 if(top.location.href.indexOf('user_index.aspx')<=0)
 {
	//alert(self.location.href);	
	if(self.location.href.indexOf('user_index.aspx')<=0&&self.location.href.indexOf('user_index.aspx')<=0&&self.location.href.indexOf('user_index.aspx')<=0&&self.location.href.indexOf('user_index.aspx')<=0)
	{
		var aa=self.location.href.split("/");
		var bb=aa[aa.length-1];
		bb=bb.replace(/\&/g,'$');
		top.location.href="user_index.aspx?url=" + bb;
	}
  }
*/
...
function setCheckStateW(objCheck)
{
    var es = document.all.tags("input") ;
	for (var i=0;i<es.length;i++)
	{
...
//document.write('<SCRIPT src="../JS/boxover.js"><\/script>');
